...
public class Campo extends StackPane {
    ...
    public Campo(ValueBox contatore) {
        ...
        //event handler per mescolare i DADI. Usa un flag settato dal dado quando il dado gestisce il handler. Il flag serve per capire se l'evento è passato attraverso il dado.
        // vengono consumati i lanci del contatore
        this.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler() {
            @Override
            public void handle(Event event) {
                for (Node e : dadi.getChildren()) {
                    if (e instanceof Dado) {
                        if (((Dado) e).isTriggered() && contatore.getN() > 0) {
                            ((Dado) e).roll(Commons.ran.nextInt(6) + 1);
                            contatore.decrement();
                            checkWin();
                        }
                    }
     ...
     * numero di dadi attivi viene incrementato. Massimo 3. Non vengono consumati i lanci del contatore
     */
    public void mostraDado() {
        if (dadiAttivi < 3) {
            dadiAttivi++;
            dadi.getChildren().add(new Dado(Commons.ran.nextInt(6) + 1));
            StackPane.setAlignment(dadi, Pos.CENTER);
        }
        checkWin();
    }
